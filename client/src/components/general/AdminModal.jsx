...
const ApprovalModal = ({ item, isOpen, onRequestClose, onApprove }) => {
  ...
  const approveWithdrawal = async (item) => {
    ...
    const getWithdrawalDetails = await axios.post("https://stinghike.com/api/administ/get_withdrawl_data/",
            {
    ...
    if(withdrawalDetails.status==="Success") {
      const getWithdraw = await axios.post("https://stinghike.com/payment/withdraw", 
      {
      ...
      if(getWithdraw.data.status_code === 1){
        ...
        const response = await axios.post(
          "https://stinghike.com/api/administ/approve_withdraw/",
          {
            withdrawal_id: item.id,
            is_rejected: false,
            transaction_id: transaction_id,
